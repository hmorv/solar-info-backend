...
exports.getHistory = async (req, res) => {
  ...
  try {
    ...
    if (range === '3d') {
      ...
      const step = Math.max(1, Math.ceil(total / 72));
      ...
      const step = Math.max(1, Math.ceil(total / 168));
...
function durationMinutesAboveThreshold(
  rows,
  field,
  threshold,
...
function peakAgeMinutes(
  ...
  return Math.max(
    0,
    Math.round(
      (referenceNow.getTime() - peakTs.getTime()) /
      60000
    )
  );
}
...
exports.getNowContext = async (req, res) => {
  logger.info(
    '➡️ Received GET request /api/solar/now-context',
    {
      query: req.query,
    }
  );
  ...
  /*
   * Se mantiene una referencia temporal JS para calcular edades
   * y duraciones a partir de los registros recuperados.
   *
   * Las ventanas SQL, sin embargo, se calculan directamente
   * mediante NOW() en MariaDB para evitar desfases UTC/local.
   */
  const now = new Date();
  ...
  try {
    ...
    const [rows25] = await conn.execute(
      `SELECT timestamp, k_index, solar_wind, magnetic_field
       FROM solar_readings
       WHERE timestamp >= DATE_SUB(NOW(), INTERVAL 25 MINUTE)
       ORDER BY timestamp ASC`
    );
    ...
    //
    // minutes solo puede tomar valores definidos en WINDOW_MINUTES
    // o el fallback fijo de 360, por lo que su interpolación aquí
    // no procede de entrada libre del usuario.
    ...
    // Median 25m
    const kMedian25 = median(
      rows25.map((r) => Number(r.k_index))
    );

    const swMedian25 = median(
      rows25.map((r) => Number(r.solar_wind))
    );

    const bMedian25 = median(
      rows25.map((r) => Number(r.magnetic_field))
    );

    const kMedianWindow = median(
      winRows.map((r) => Number(r.k_index))
    );

    // Slope 60m
    const kSlope60 = slopePerHour(
      rows65.map((r) => ({
        t: r.timestamp,
        v: Number(r.k_index),
      }))
    ...
    const swSlope60 = slopePerHour(
      rows65.map((r) => ({
        t: r.timestamp,
        v: Number(r.solar_wind),
      }))
    );
    ...
    // Picos y persistencia
    const kVals = winRows
      .map((r) => Number(r.k_index))
      .filter(Number.isFinite);

    const pVals = winRows
      .map((r) => Number(r.proton_flux))
      .filter(Number.isFinite);

    const maxK = kVals.length
      ? Math.max(...kVals)
      : null;

    const maxP = pVals.length
      ? Math.max(...pVals)
      : null;

    const minutesKge4 =
      durationMinutesAboveThreshold(
        winRows,
        'k_index',
        4,
        now
    ...
    const xrayCurrentClass =
      xrayClass(xrayCurrent);
    ...
    /*
     * Coverage y confidence.
     *
     * El collector trabaja aproximadamente cada 5 minutos.
     * Coverage representa cobertura de la ventana, por lo que
     * su máximo semántico es 1 (100%).
     */
    const expected =
      Math.max(
        1,
    ...
    logger.info(
      'Response sent for /api/solar/now-context',
      {
        window,
        trend,
        confidence,
    ...
    logger.error(
      'Error in /api/solar/now-context',
      {
        error: err,
      }
    );
